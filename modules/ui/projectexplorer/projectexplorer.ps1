...
properties{
	...
	# solution files
	$slnProjectExplorer = Join-Path $dirSrc 'Apollo.ProjectExplorer.sln'
	
	$msbuildStyleCop = Join-Path $dirTemplates 'StyleCop.msbuild'
	$msbuildApiDoc = Join-Path $dirBase 'Apollo.ProjectExplorer.shfbproj'
	...
	# output files
	$logMsiBuild = 'projectexplorer_msi.log'
	$logMsBuild = 'projectexplorer_msbuild.log'
	$logFxCop = 'projectexplorer_fxcop.xml'
	$logNCover = 'projectexplorer_ncover.xml'
	$logNCoverHtml = 'projectexplorer_ncover.html'
...
In order to run this build script please call this script via PSAKE like:
	invoke-psake projectexplorer.ps1 incremental,debug,clean,build,unittest,verify -framework 4.0 -timing -noexit -showfullerror
"@
}
...
task runClean -precondition{ $shouldClean } -action{
	...
	$msbuildExe = Get-MsbuildExe
	& $msbuildExe $slnProjectExplorer /t:Clean /verbosity:minimal
...
task buildBinaries -depends runInit, getVersion -action{
	"Building Apollo.ProjectExplorer..."
	...
	$msbuildExe = Get-MsbuildExe
	Invoke-MsBuild $slnProjectExplorer $configuration $logPath 'minimal' ("platform='Any CPU'")
	if ($LastExitCode -ne 0)
	{
		throw "Apollo.ProjectExplorer build failed with return code: $LastExitCode"
	}
	
	# Copy the binaries
	$dirBinProjectExplorer = Join-Path (Join-Path (Join-Path $dirSrc 'ProjectExplorer') 'bin') $configuration
	$dirBinTestIntegration = Join-Path (Join-Path (Join-Path $dirSrc 'ProjectExplorer.Test.Integration') 'bin') $configuration

	Copy-Item (Join-Path $dirBinProjectExplorer '*') $dirBuild -Force
	Copy-Item (Join-Path $dirBinTestIntegration '*') $dirBuild -Force
}
...
#	if ($LastExitCode -ne 0)
#	{
#		throw "MbUnit failed on Apollo.ProjectExplorer with return code: $LastExitCode"
#	}
#	
...
#		if ($LastExitCode -ne 0)
#		{
#			throw "NCoverExplorer failed on Apollo.ProjectExplorer with return code: $LastExitCode"
#		}
#	}
}
...
task buildApiDoc -depends buildBinaries -action{
	...
	if ($LastExitCode -ne 0)
	{
		throw "Sandcastle help file builder failed on Apollo.ProjectExplorer with return code: $LastExitCode"
	}
...
task runStyleCop -depends buildBinaries -action{
	...
	if ($LastExitCode -ne 0)
	{
		throw "Stylecop failed on Apollo.ProjectExplorer with return code: $LastExitCode"
	}
...
task runFxCop -depends buildBinaries -action{
	...
	if ($LastExitCode -ne 0)
	{
		throw "FxCop failed on Apollo.ProjectExplorer with return code: $LastExitCode"
	}
}
...
task buildPackage -depends buildBinaries -action{
	...
	if ($LastExitCode -ne 0)
	{
		throw "Failed to compress the Apollo.ProjectExplorer binaries."
	}
}