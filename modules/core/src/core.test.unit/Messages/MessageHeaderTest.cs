...
namespace Apollo.Core.Test.Unit.Messages
{
    ...
    {
        [Test]
        [Description("Checks that a header can be created with a sender and an ID.")]
        public void CreateWithSenderAndMessageId()
        {
            var id = MessageId.Next();
            var sender = new DnsName("name");

            var header = new MessageHeader(id, sender);
            Assert.AreSame(id, header.Id);
            Assert.AreSame(sender, header.Sender);
            Assert.AreEqual(DnsName.AllServices, header.Recipient);
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new MessageHeader(null, new DnsName("name")));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new MessageHeader(MessageId.None, new DnsName("name")));
        }
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new MessageHeader(MessageId.Next(), null));
        }

        [Test]
        [Description("Checks that a header cannot be created with a sender equal to the AllServices ID.")]
        public void CreateWithAllServicesSender()
        {
            Assert.Throws<ArgumentException>(() => new MessageHeader(MessageId.Next(), DnsName.AllServices));
        }
        ...
        {
            Assert.Throws<ArgumentException>(() => new MessageHeader(MessageId.Next(), DnsName.Nobody));
        }
        ...
        public void EqualsWithNonEqualObject()
        {
            ...
            var header = new MessageHeader(id, sender);
            Assert.IsFalse(header.Equals(new MessageHeader(MessageId.Next(), sender)));
        }
