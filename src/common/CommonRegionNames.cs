//-----------------------------------------------------------------------
// <copyright company="P. van der Velde">
//     Copyright (c) P. van der Velde. All rights reserved.
// </copyright>
//-----------------------------------------------------------------------

namespace Apollo.UI.Common
{
    /// <summary>
    /// Defines the common PRISM regions.
    /// </summary>
    public static class CommonRegionNames
    {
        /// <summary>
        /// Defines the name of the top pane region of the project view.
        /// </summary>
        public const string ProjectViewTopPane = "Common_ProjectTopPane";

        /// <summary>
        /// Defines the name of the content region of the project view.
        /// </summary>
        public const string ProjectViewContent = "Common_ProjectContent";

        /// <summary>
        /// Defines the name of the content region of the shell.
        /// </summary>
        public const string Content = "Content";
    }
}
