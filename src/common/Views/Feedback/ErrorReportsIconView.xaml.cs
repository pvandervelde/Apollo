...
namespace Apollo.UI.Common.Views.Feedback
{
    ...
    public partial class ErrorReportsIconView : UserControl, IErrorReportsView
    {
        ...
        {
            var items = from FeedbackReportModel report in errorReportsListBox.SelectedItems
                        select new FileInfo(report.Path);

            e.Handled = true;
            e.CanExecute = Model != null ? Model.SendReportsCommand.CanExecute(items) : false;
        }
        ...
        private void CommandSendReportsExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            e.Handled = true;
            Model.SendReportsCommand.Execute(items);
            Model.RelocateReports();
        }
