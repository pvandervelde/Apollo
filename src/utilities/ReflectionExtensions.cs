...
namespace Apollo.Utilities
{
    ...
    public static class ReflectionExtensions
    {
        ...
        /// <param name="name">The assembly name that should be compared to the current assembly name.</param>
        /// <returns>
        ...
        public static bool IsSame(this AssemblyName current, AssemblyName name)
        {
            var isMatch = string.Equals(current.Name, name.Name, StringComparison.Ordinal);
            isMatch = isMatch
                && (((current.CultureInfo != null) && current.CultureInfo.Equals(name.CultureInfo))
                || ((current.CultureInfo == null) && name.CultureInfo == null));

            isMatch = isMatch && current.Version.Equals(name.Version);
            isMatch = isMatch && current.GetPublicKey().SequenceEqual(name.GetPublicKey());

            return isMatch;
        }

        /// <summary>
        /// Returns a value indicating if the given assembly name belongs to the same assembly as the assembly with the current name, or
        /// a later version of that assembly.
        /// </summary>
        /// <param name="current">The current assembly name.</param>
        /// <param name="name">The assembly name that should be compared to the current assembly name.</param>
        /// <returns>
        /// <see langword="true" /> if the given assembly name belongs to the same assembly as the assembly with the current name, or
        /// a later version of that assembly; otherwise, <see langword="false"/>.
        /// </returns>
        [SuppressMessage("Microsoft.StyleCop.CSharp.DocumentationRules", "SA1628:DocumentationTextMustBeginWithACapitalLetter",
            Justification = "Documentation can start with a language keyword")]
        public static bool IsMatch(this AssemblyName current, AssemblyName name)
        {
            var isMatch = string.Equals(current.Name, name.Name, StringComparison.Ordinal);
            isMatch = isMatch
                && (((current.CultureInfo != null) && current.CultureInfo.Equals(name.CultureInfo))
            ...
            if (current.GetPublicKey().Length > 0)
            {
                isMatch = isMatch
                    && current.GetPublicKey().SequenceEqual(name.GetPublicKey())
                    && current.Version.Equals(name.Version);
            }
            else 
            {
                isMatch = isMatch && (current.Version <= name.Version);
            }
